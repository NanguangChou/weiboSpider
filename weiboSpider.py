...
class weibo:
	cookie = {"cookie":"SCF=ArUHCyDNpRI9ErdaQ2RtfeFDmGNjOk1mlzxkR-aK9OAKop_693wIsRdo6Ftz-iHM3E_l9rhxGlrPNMWxwx7SPlU.; _T_WM=2d2256ecf464663dc45325ac1d6f8194; SUB=_2A251P5wyDeTxGeRL7FYR8SzEzT2IHXVWwyR6rDV6PUJbkdBeLVn8kW0bCC6WpVDlIW1mKpChEPoC1W55HQ..; SUHB=0V5Unp9c_NpC0D; SSOLoginState=1480322146; M_WEIBOCN_PARAMS=uicode%3D20000174"}
	#weibo类初始化
		...
		for page in range(1,5):
		  ...
		  if len(info) > 3:
			for i in range(0,len(info)-2):
			  ...
			  time = info[i].xpath("div/span/text()")
			  timestr = time[1].encode('utf-8')
			  # print timestr
			  # self.send_time.append((timestr).encode('gbk'))
			  # print etree.tostring(info[i])
	...
	def writeTxt(self):
	  try:
		...
		for i in range(1,self.weiboNum2 + 1):
		  text=str(i) + ':' + self.weibos[i-1] + '\n'+'	 转发数：' + str(self.num_forwarding[i-1]) + '	 评论数：' + str(self.num_comment[i-1]) + '   发送时间:' + "str(self.send_time[i-1])" +'\n\n'
		  result = result + text
...
user_id = 1884326311 #可以改成任意合法的用户id（爬虫的微博id除外）
